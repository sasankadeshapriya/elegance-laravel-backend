...
class ProductCartController extends Controller
{
    ...
    public function OrderSummery(){
        $orders = CartOrder::whereIn('order_status', ['Pending', 'Complete', 'Processing'])
                            ->orderBy('id', 'DESC')
                            ->get();
    ...
    public function PendingToProcessing($id) {
        ...
        // Update quantity in product_lists table
        $productList = ProductList::where('product_code', $productCode)->first();
        if ($productList) {
            $productList->decrement('quantity', $quantity);
        }

        // Update quantity in product_details table
        $productDetails = ProductDetails::where('product_code', $productCode)->first();
        if ($productDetails) {
